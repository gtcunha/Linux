...
# Verificar se as variáveis essenciais estão preenchidas
if [[ -z "$DB_USER" || -z "$DB_PASSWORD" || -z "$DB_HOST" || -z "$DB_NAME" || -z "$BACKUP_DIR" ]]; then
    echo "Erro: Verifique as variáveis de configuração. Nenhuma delas pode estar vazia."
    exit 1
fi
